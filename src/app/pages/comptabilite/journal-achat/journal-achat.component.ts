...
@Component({
  selector: 'journal-achat',
  templateUrl: './journal-achat.component.html',
  styleUrls: ['./journal-achat.component.scss']
})
export class JournalAchatComponent implements OnInit, OnDestroy, AfterViewInit {
  @Input() nameModule
  ...
  ngOnInit() {
    ...
    this.translate.get("labels").subscribe(labels => {
      this.journalVenteTableColumns = [labels.codeJournal, labels.date, labels.numeroCompte, labels.numeroPiece, labels.fournisseur, labels.debit, labels.credit];
    })
  }
  ...
  exporter() {
    this.loading = true;
    this.comptabilteService.ExporterJournalAchat(
      "",
      1,
      ...
      value => {
        ...
        // file extension
        var extension = "xlsx";
        AppSettings.setFile(fileData, ("JournalAchat") + stamp + "." + extension, fileType, extension);
      },
      err => {
        this.loading = false;
