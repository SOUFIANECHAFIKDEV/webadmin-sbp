...
import { Devis, FactureSituationDevis } from 'app/Models/Entities/Devis';
...
@Component({
  selector: 'app-add',
  templateUrl: './add-facture-situation.component.html',
  styleUrls: ['./add-facture-situation.component.scss']
})

export class AddFactureSituationComponent implements OnInit {
  ...
  setAdresse() {
    ...
    while (typeof (this.adresseFacturation) == "string") {
      this.adresseFacturation = JSON.parse(this.adresseFacturation);
    }
  ...
  */
  ...
  async add(statut: StatutFacture) {
    ...
    this.prestations = await this.getDataFromArticlesComponet(statut);
  ...
  async createBodyRequest(status: StatutFacture): Promise<any> {
    ...
    facture.typeFacture = TypeFacture.Situation;
    ...
    infoClient.adresseFacturation = (JSON.parse(client.adresses) as Adresse[]).filter(X => X.default == true)[0];
    facture.infoClient = JSON.stringify(infoClient);


    /*    if (status == this.statutFacture.Cloture) {
          facture.totalHt = this.prestations.situationCumulleeHT;
          facture.total = this.prestations.situationCumulleeTTC;
    ...
    // facture.total = this.calcTotalTtcToSave(status, this.prestations.totalTtc, totalHt, this.prestations.tva);
    ...
    return {
      ...
      situations: this.createFacturesSituationInfos(status, pourcentage),
      facture: facture
    }
  }

  createFacturesSituationInfos(status, pourcentage): FactureSituationDevis {
    ...
    return {
      ...
      situationCumulleeTTC: this.prestations.situationCumulleeTTC,
      situationCumulleeHT: this.prestations.situationCumulleeHT,
